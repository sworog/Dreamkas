...
Scenario: Проверка, что после удаления поставщика нельзя изменить уже созданную накладную с ним
...
When пользователь* вводит значение 'user-supplier' в поле с именем 'supplier'

When пользователь* нажимает на кнопку создания 'Принять'

Then пользователь видит сообщение об ошибке 'Не могу так сделать, брат'

Scenario: Проверка, что после удаления поставщика нельзя изменить уже созданный возврат поставщика с ним
