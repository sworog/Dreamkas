...
Narrative:
Как владелец,
Я хочу создавать, редактировать и удалять возвраты товаров поставщику в магазинах,
...
| 08.11.2014 | Возврат поставщику | В s40u105-store | 750,00 |
...
Then пользователь* проверяет, что список товаров содержит товары с данными
| name | price | quantity | totalPrice |
| s40u105-product1 | 11,99  | 2,0 шт. | 23,98 |
...
Then пользователь проверяет операции на странице товародвижения
| date | type | store | sumTotal |
| 08.11.2014 | Возврат поставщику | В s40u105-store1 | 149,48 |
...
Then пользователь* в модальном окне проверяет, что заголовок равен 'Редактирование возврата поставщику'
...
Then пользователь проверяет операции на странице товародвижения
| date | type | store | sumTotal |
| 08.11.2014 | Возврат поставщику | В s40u105-store1 | 125,50 |
