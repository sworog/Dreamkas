...
Then пользователь* проверяет, что список содержит точные данные
| price | date |
...
Given пользователь создает чек с наличным типом оплаты, внесенной суммой '1000' и датой 'saleTodayDate'
