...
When пользователь* находится в модальном окне 'модальное окно создания оприходования'
...
| price | 150 |
| quantity | 5 |
And пользователь нажимает на кнопку добавления нового товара в оприходование

Then пользователь проверяет, что список товаров в оприходовании содержит товары с данными
| name | priceEntered | quantity | totalPrice |
| s39u102-product1 | 150,00  | 5,0 шт. | 750,00 |
And пользователь проверяет, что сумма итого равна '750,00' в модальном окне создания оприходования
...
When пользователь* находится в модальном окне 'модальное окно редактирования оприходования'
...
| cause | причина |
And пользователь нажимает на кнопку добавления нового товара в оприходование в модальном окне редактирования оприходования

Then пользователь проверяет, что список товаров в оприходовании содержит товары с данными
| name | price | quantity | totalPrice |
| s40u104-product1 | 11,99  | 2,0 шт. | 23,98 |
| s40u104-product2 | 125,50  | 1,0 Пятюня | 125,50 |
And пользователь проверяет, что сумма итого равна '149,48' в модальном окне редактирования оприходования
...
And пользователь нажимает на кнопку удаления оприходования в модальном окне редактирования оприходования
And пользователь подтверждает удаление оприходования в модальном окне редактирования оприходования
...
When пользователь нажимает на кнопку Оприходовать на странице товародвижения
And пользователь* находится в модальном окне 'модальное окно создания оприходования'

Then пользователь* проверяет, что заголовок модального окна равен 'Оприходование товаров'
...
When пользователь нажимает на последнее созданное оприходование с помощью конструктора оприходований на странице товародвижения
And пользователь* находится в модальном окне 'модальное окно редактирования оприходования'

Then пользователь* проверяет, что заголовок модального окна равен 'Редактирование оприходования товаров'
...
When пользователь удаляет товар с названием 's40u104-product1' в модальном окне редактирования оприходования

Then пользователь проверяет, что список товаров в оприходовании содержит товары с данными
| name | price | quantity | totalPrice |
| s40u104-product2 | 125,50  | 1,0 Пятюня | 125,50 |
And пользователь проверяет, что сумма итого равна '125,50' в модальном окне редактирования оприходования
