...
Then пользователь* проверяет, что список 'денежных операций' не содержит элемент с именем 'todayDate:+ 1 500,00'
...
| todayDate-1 | + 1 500,00 | Продажи от todayDate-1 |

Scenario: Проверка пересчета продаж в денежной операции при приходе продажи задним числом
...
Given пользователь создает чек c датой 'saleTodayDate-1'
And пользователь добавляет товар в чек с именем 'user-product1', количеством '1' и по цене '150'
And пользователь вносит наличные в размере '150' рублей
And пользователь с адресом электронной почты 'user@lighthouse.pro' в магазине с именем 'store-user' совершает продажу по созданному чеку
...
Then пользователь* проверяет, что список 'денежных операций' содержит точные данные
| Дата | Сумма | Комментарий |
| todayDate-1 | + 1 650,00 | Продажи от todayDate-1 |