...
class GrossSalesControllerTest extends WebTestCase
{
    ...
    public function testGrossSalesForPeriod()
    {
        $store = $this->factory()->store()->getStore();
        ...
        $this->factory()
            ->receipt()
                ->createSale($store, '8:01')
                ->createReceiptProduct($productIds['1'], 3, 34.77)  // 104.31
                ->createReceiptProduct($productIds['2'], 3, 64.79)  // 194.37
                ->createReceiptProduct($productIds['3'], 7, 43.55)  // 304.85
            ->persist()                                             // 603.53
                ->createSale($store, '9:01')
                ->createReceiptProduct($productIds['1'], 3, 34.77)
                ->createReceiptProduct($productIds['2'], 3, 64.79)
            ...
            ->persist()
                ...
                ->createReceiptProduct($productIds['3'], 7, 43.55)
            ->persist()
                ->createSale($store, '-1 days 11:01')
                ->createReceiptProduct($productIds['1'], 3, 34.77)
                ->createReceiptProduct($productIds['2'], 3, 64.79)
                ->createReceiptProduct($productIds['3'], 7, 43.55)
            ->persist()
                ->createSale($store, '-2 days 8:01')
                ->createReceiptProduct($productIds['1'], 4, 34.77)  // 139.08
                ->createReceiptProduct($productIds['2'], 3, 64.79)
                ->createReceiptProduct($productIds['3'], 7, 43.55)
            ->persist()
                ->createSale($store, '-2 days 9:01')
                ->createReceiptProduct($productIds['1'], 3, 34.77)
                ->createReceiptProduct($productIds['2'], 3, 64.79)
                ->createReceiptProduct($productIds['3'], 7, 43.55)
            ->persist()
            ...
            ->persist()
                ...
                ->createReceiptProduct($productIds['2'], 3, 64.79)
                ->createReceiptProduct($productIds['3'], 7, 43.55)
                ->createReceiptProduct($productIds['3'], 3, 34.00)  // 102
            ->persist()                                             // 705.53
        ...
        $response = $this->clientJsonRequest(
            $accessToken,
            'GET',
            "/api/1/stores/{$store->id}/reports/grossMarginSalesForPeriod",
            null,
            array(
                'startDate' => date('c', strtotime('-3 days 00:00:00')),
                'endDate' => date('c', strtotime('10:35:47')),
            )
        );
        ...
        $expectedProduct1Today = array(
            ...
            'date' => date('c', strtotime('00:00:00')),
            'grossSales' => 208.62,
            'quantity' => 6
        );
        Assert::assertJsonPathArrayContains($expectedProduct1Today, '*', $response);
        ...
        $expectedProduct1Yesterday = array(
            ...
            'date' => date('c', strtotime('-1 days 00:00:00')),
            'grossSales' => 208.62,
            'quantity' => 6
        );
        Assert::assertJsonPathArrayContains($expectedProduct1Yesterday, '*', $response);

        $expectedProduct1DaysAgo2 = array(
            'storeProduct' => array(
                'product' => array(
                    'id' => $productIds['1']
                )
            ),
            'date' => date('c', strtotime('-2 days 00:00:00')),
            'grossSales' => 243,39,
            'quantity' => 7
        );
        Assert::assertJsonPathArrayContains($expectedProduct1DaysAgo2, '*', $response);

        $expectedProduct1DaysAgo3 = array(
            'storeProduct' => array(
                'product' => array(
                    'id' => $productIds['1']
                )
            ),
            'date' => date('c', strtotime('-3 days 00:00:00')),
            'grossSales' => 208.62,
            'quantity' => 6
        );
        Assert::assertJsonPathArrayContains($expectedProduct1DaysAgo3, '*', $response);

        Assert::assertJsonPathEquals($productIds['2'], '*.storeProduct.product.id', $response, 4);
        Assert::assertJsonPathEquals($productIds['3'], '*.storeProduct.product.id', $response, 4);

        /*
        {
            "storeProduct": {
                "store": {
                    "id": "dkfjhkjef34r2348hkdsf"
                }
            }
            "date": "2013-11-11T23:00:00+0400",
            "quantity" : 77,
            "grossSales": 24000.34,
            "grossMargin": 10000,
            "grossProfit": 14000.34
        }
         */
