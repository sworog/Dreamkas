...
class ReturnProductControllerTest extends WebTestCase
{
    public function testGetProductReturnProducts()
    {
        ...
        $products = $this->factory()->catalog()->getProductByNames(array('1', '2', '3', '4'));

        $saleObject1 = $this->factory()
            ->receipt()
                ->createSale($store, '2012-05-12T00:00:00.000+04:00')
                ->createReceiptProduct($products['3']->id, 100, 513)
                ->createReceiptProduct($products['4']->id, 25, 180)
                ->createReceiptProduct($products['1']->id, 1.57, 36)
                ->createReceiptProduct($products['3']->id, 3.576, 36)
            ->flush();

        $this->factory()
            ->receipt()
                ->createReturn($store, '2012-05-12T19:31:44.492+04:00', $saleObject1)
                ->createReceiptProduct($products['1']->id, 1) //36
                ->createReceiptProduct($products['3']->id, 25) //36
            ->flush();

        $this->factory()
            ->receipt()
                ->createReturn($store, '2012-05-12T19:46:32.912+04:00', $saleObject1)
                ->createReceiptProduct($products['4']->id, 1) //180
            ->flush();
        ...
        $getResponse1 = $this->clientJsonRequest(
            $accessToken,
            'GET',
            "/api/1/stores/{$store->id}/products/{$products['1']->id}/returnProducts"
        );
        ...
        Assert::assertNotJsonPathEquals($products['2']->id, '*.product.id', $getResponse1);
        Assert::assertNotJsonPathEquals($products['3']->id, '*.product.id', $getResponse1);
        Assert::assertNotJsonPathEquals($products['4']->id, '*.product.id', $getResponse1);
        Assert::assertJsonPathEquals($products['1']->id, '0.product.id', $getResponse1);
        ...
        $getResponse2 = $this->clientJsonRequest(
            $accessToken,
            'GET',
            "/api/1/stores/{$store->id}/products/{$products['2']->id}/returnProducts"
        );
        ...
        $getResponse3 = $this->clientJsonRequest(
            $accessToken,
            'GET',
            "/api/1/stores/{$store->id}/products/{$products['3']->id}/returnProducts"
        );

        $this->assertResponseCode(200);

        Assert::assertJsonPathCount(1, '*.id', $getResponse3);
        Assert::assertNotJsonPathEquals($products['1']->id, '*.product.id', $getResponse3);
        Assert::assertNotJsonPathEquals($products['2']->id, '*.product.id', $getResponse3);
        Assert::assertNotJsonPathEquals($products['4']->id, '*.product.id', $getResponse3);
        Assert::assertJsonPathEquals($products['3']->id, '0.product.id', $getResponse3);
        ...
        $getResponse4 = $this->clientJsonRequest(
            $accessToken,
            'GET',
            "/api/1/stores/{$store->id}/products/{$products['4']->id}/returnProducts"
        );

        $this->assertResponseCode(200);

        Assert::assertJsonPathCount(2, '*.id', $getResponse4);
        Assert::assertNotJsonPathEquals($products['1']->id, '*.product.id', $getResponse4);
        Assert::assertNotJsonPathEquals($products['2']->id, '*.product.id', $getResponse4);
        Assert::assertNotJsonPathEquals($products['3']->id, '*.product.id', $getResponse4);
        Assert::assertJsonPathEquals($products['4']->id, '*.product.id', $getResponse4, 2);
