...
function( tagName, factory ){
        if(!( this instanceof $jam_Component )) return new $jam_Component( tagName, factory )
        ...
        $jam_domReady.then(function whenReady(){
        ...
        docEl.addEventListener( 'DOMNodeInserted', function whenNodeInserted( ev ){
            ...
            //$jam_schedule( 0, function( ){
                check4attach([ node ])
                if( !$jam_support.msie() && node.getElementsByTagName ) check4attach( node.getElementsByTagName( tagName ) )
            //})
        }, false )
        docEl.addEventListener( 'DOMNodeRemoved', function whenNodeRemoved( ev ){
            ...
            //$jam_schedule( 0, function( ){
                check4detach([ node ])
                if( !$jam_support.msie() && node.getElementsByTagName ) check4detach( node.getElementsByTagName( tagName ) )
            //})
        }, false )
        
        this.tagName= $jam_Value( tagName )
        this.factory= $jam_Value( factory )
        this.elements=
        function elements( ){
...
$jam_Class( function( klass, proto ){
        ...
        ,   function( keyList ){
                ...
                for( var i= 0; i < keyList.length; ++i ){
                    var key= keyList[ i ]
                    cur= cur[ key ]
                    if( $jam_classOf( cur ) !== 'Object' ) break
                }
                return cur
            }
        )
...
$jam_Class( function( klass, proto ){
        ...
        $jam_Poly
        (   function( ){
                return $jam_html2text( this.$.innerHTML )
            }
        ,   new function(){
                return function( val ){
        ...
        ,   function( key ){
                return $jam_Hiqus({ splitterChunks: ' ' }).merge( this.$.className || '' ).get( key )
            }
        ,   function( key, value ){
                this.$.className= $jam_Hiqus({ splitterChunks: ' ' }).merge( this.$.className ).put( key, value )
                return this
            }
        )
        ...
        $jam_Poly
        (   function( ){
                return $jam_Node( this.$.parentNode )
            }
        ,   function( node ){
                node= $jam_raw( node )
        ...
        function( ){
            ...
            //if( this.name() === 'br' ) return this;//this.prev( $jam_Node.Text( '\r\n' ) )
            this.parent( null )
            return this
        }
        ...
        $jam_Poly
        (   function(){
                return $jam_Node( this.$.firstChild )
            }
        ,   function( node ){
                this.$.insertBefore( $jam_raw( node ), this.$.firstChild )
                return this
            }
        )
        ...
        $jam_Poly
        (   function(){
                return $jam_Node( this.$.lastChild )
            }
        ,   function( node ){
                this.$.appendChild( $jam_raw( node ) )
                return this
            }
        )
        ...
        $jam_Poly
        (   function(){
                return $jam_Node( this.$.nextSibling )
            }
        ,   function( node ){
                var parent= this.$.parentNode
                var next= this.$.nextSibling
                parent.insertBefore( $jam_raw( node ), next ) 
                return this
            }   
        )
        ...
        $jam_Poly
        (   function(){
                return $jam_Node( this.$.previousSibling )
            }
        ,   function( node ){
                node= $jam_raw( node )
                var parent= this.$.parentNode
        ...
        new function( ){
            ...
            return function( html ){
                ...
                var fragment= $jam_Node.Fragment()
        ...
        proto.clone=
        function( ){
            return $jam_Node( this.$.cloneNode( false ) )
        }

        proto.cloneTree=
        function( ){
            return $jam_Node( this.$.cloneNode( true ) )
        }
...
$jam_Class( function( klass, proto ){
        ...
        function( ){
            ...
            for( var i= 0; i < arguments.length; ++i ){
                ...
                if( i % 2 ) chunk= $jam_RegExp.escape( chunk )
                str+= chunk
            }
            return $jam_RegExp( str )
        }
...
function( lexems ){
        ...
        for( var name in lexems ){
            var regexp= $jam_RegExp( lexems[ name ] )
            nameList.push( name )
        ...
        var count= $jam_RegExp(regexp).count()
...
$jam_Class( function( klass, proto ){
        ...
        function( syntax ){
            ...
            var lineParser= $jam_RegExp.build( '^((?:', oneIndent, ')*)(.*?)(?:', valSep, '(.*))?$' ).$
