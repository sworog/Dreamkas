...
public class ButtonFlatExt extends ButtonFlat {
    ...
    private void setAttrs(Context context, AttributeSet attrs) {
        ...
        getTextView().setGravity(TEXT_ALIGNMENT_CENTER);

        int[] textColorAttr = new int[] { android.R.attr.textColor };
        TypedArray a1 = context.obtainStyledAttributes(attrs, textColorAttr);
        int color = a1.getColor(0, -1);

        if(textSize != -1){
            getTextView().setTextColor(color);
        }else{
            getTextView().setTextSize(Color.BLACK);
        }
